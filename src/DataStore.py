    ...
    def __index_by_id(self, id_index, data):
        if '_id' not in data:
            raise InvalidDataError(
                f"Data missing '_id' discovered within data set: {self.model.name}")

        id_to_index = str(data['_id'])
        if id_to_index not in id_index:
            id_index[id_to_index] = data
            return id_index
        else:
            raise InvalidDataError(
                f"Duplicate primary _id ({id_to_index}) discovered within data set: {self.model.name}")

    def __index_by_field(self, field_index, data):
        for field in data:
            if field not in self.model.possible_fields:
                raise InvalidDataError(
                    f'Invalid data key ({field}) discovered in model: {self.model.name}')

            # Managing list fields
            if isinstance(data[field], list):
                for list_entry in data[field]:
                    if list_entry not in field_index[field]:
                        field_index[field][str(list_entry)] = [
                            str(data['_id'])]
                    else:
                        field_index[field][str(list_entry)].append(
                            str(data['_id']))

            # Managing other fields
            else:
                if str(data[field]) in field_index[field]:
                    field_index[field][str(data[field])].append(
                        str(data['_id']))
                else:
